...
### Our models ####
function create_vsm_gfm(static_device)
    return DynamicInverter(;
        name = get_name(static_device),
        ω_ref = 1.0, # ω_ref,
        converter = converter_high_power(), #converter
...
function create_gfl(static_device)
...
function create_gfl_lower_pi(static_device)
    return PSY.DynamicInverter(
        get_name(static_device),
        1.0, #ω_ref
        converter_low_power(), #converter
        outer_control_gfoll2(), #outercontrol
        current_mode_inner(), #inner_control
        dc_source_lv(),
        reduced_pll(),
        filt_gfoll(),
...
avr30() = AVRFixed(V_ref=1.0, Vf=1.0)
tg30() = TGFixed(P_ref=1.0, efficiency=1.0)
shaft30() = SingleMass(D=0.0, H=21.0)
mach30() = RoundRotorQuadratic(
        R = 0.0,
        Td0_p = 10.2,
        Td0_pp = 0.05,
        Tq0_p = 0.01,
        ...
        Xd_pp = 0.05,
        Xl = 0.025,
        Se = (0.0, 0.0) # sat coeffs?
)

function dyn_gen_roundrotor30(generator)
    return PSY.DynamicGenerator(;
    name = get_name(generator),
    ω_ref = 1.0, 
    machine = mach30(),
...
function dyn_gen_marconato_simple(generator)
    return PSY.DynamicGenerator(;
        name = get_name(generator),
        ω_ref = 1.0, # ω_ref,
        machine = machine_simple_marconato(), #machine
        shaft = shaft_no_damping(), #shaft
